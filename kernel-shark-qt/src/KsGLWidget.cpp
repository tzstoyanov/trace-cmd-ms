...
void KsGLWidget::loadData(KsDataStore *data)
{
	...
	_model.fill(_data->pevent(), _data->rows(), _data->size());
...
{
	return graphCount()*(KS_GRAPH_HEIGHT + _vSpacing) + _vMargin * 2;
}
...
void KsGLWidget::makeGraphs(QVector<int> cpuList, QVector<int> taskList)
{
	...
	auto add_graph = [&] (KsPlot::Graph *graph) {
		/*
...
int KsGLWidget::getCpu(int y)
{
	...
	int cpuId = (y - _vMargin + _vSpacing / 2) / (_vSpacing + KS_GRAPH_HEIGHT);
