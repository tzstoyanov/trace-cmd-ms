...
int kshark_histo_get_pid_front(struct kshark_trace_histo *histo,
			       int bin, int cpu, bool visOnly);

int kshark_histo_get_pid_back(struct kshark_trace_histo *histo,
			      int bin, int cpu, bool visOnly);

int kshark_histo_get_cpu(struct kshark_trace_histo *histo,
			 int bin, int pid, bool visOnly);

static inline double kshark_histo_bin_time(struct kshark_trace_histo *histo, int bin)
{
	return (histo->min + bin*histo->bin_size)*1e-9;
}

static inline uint64_t kshark_histo_ts(struct kshark_trace_histo *histo, int bin)
{
