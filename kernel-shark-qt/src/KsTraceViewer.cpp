				      ...
				      condition_func cond)
{
	...
	int i, delta = _data->size() / nThreads;
	...
	maps.push_back(std::async(lamSearchMap, ranges[0], &_searchProgBar));
	for (int r = 1; r < nThreads; ++r)
		maps.push_back(std::async(lamSearchMap, ranges[r], nullptr));

	while (_proxyModel.searchProgress() < PROG_BAR_MAX - nThreads) {
